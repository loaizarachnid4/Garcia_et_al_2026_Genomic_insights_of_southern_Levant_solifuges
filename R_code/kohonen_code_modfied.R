...
c_mat[1:length(cluster_assignment), j] <- cluster_assignment
...
DNA.SOM.small <- function() {
  ## 1. Create SOM Grid (hexagonal with n x n layout)
  max <- nrow(som_grid2$pts)  # number of SOM units
  
  ##Output matrices
  c_mat <- data.frame(row.names = row.names(alleles))#to hold classifiers for DNAsom
  l_mat <- data.frame(row.names = 1:m)#to hold learning values
  w_mat <- data.frame(row.names = 1:(max-1))#to hold dWSS for K
  ...
  #calculate wss to select k
  mydata <- getCodes(som_model)#pull codebook vectors for wss
  wss <- (nrow(mydata))*sum(apply(mydata,2,var))#Get weighted sum of squares
  for (i in 2:(max-1)) {wss[i] <- sum(kmeans(mydata,centers=i,nstart=25,iter.max=1e5)$withinss)}  #Repeat kmeans to get best classification
  N <- dim(mydata)[1]#sample size for bic
  w_mat[,j] <- N*log(wss/N)+log(N)*(1:(max-1))#store bic
...
match.labels.small <- function(alleles)
{a <- alleles;a[which(is.na(a))] <- 0.5
labels <- rbind.data.frame(lapply(1:(nrow(alleles)-1),function(x){kmeans(a,x)$cluster}))
rownames(labels) <- rownames(alleles);colnames(labels) <- paste("K",1:(nrow(alleles)-1),sep='')
labels}

#match.labels.small <- function(alleles)
#{a <- alleles;a[which(is.na(a))] <- 0.5
#labels <- rbind.data.frame(lapply(1:(nrow(alleles)-1),function(x){kmeans(a,x)$cluster}))
#rownames(labels) <- rownames(alleles);colnames(labels) <- paste("K",1:nrow(w_mat),sep='')
#labels}
...
plotK.small <- function(res)
{
  ...
  #by absolute BIC
  boxplot(t(w_mat)[,-11],outline=F,notch=T,axes=F, ylab="BIC",ylim=range(unlist(w_mat[,-11])),col=k.cols)
  axis(1,at=1:(8),labels=NA);axis(2,at=round(range(unlist(w_mat[,-11]))),las=3);title("Number of clusters (k)", line=0)
  
  #by delta BIC - if only three groups are supported
  d_wss <- apply(w_mat, 2, function(x){ diff(diff(x)) })
  rownames(d_wss) <- as.character(2:(nrow(w_mat)-1))  # Reflect second diff result
  plot_dwss <- rbind(NA, d_wss)
  boxplot(t(plot_dwss), outline = FALSE, notch = TRUE, axes = FALSE, 
          ylab = "delta BIC", col = k.cols)
  abline(h = 0, lty = 2, col = "red")
  axis(1, at = 1:nrow(plot_dwss), labels = rownames(plot_dwss))
  axis(2, at = sort(c(0, round(range(unlist(d_wss))))), las = 3)
  
  #by frequency
  par(mar=c(4,4,1,0.5))
  all_k <- apply(res$c_mat,2,max) # Get the K for each run
  barplot(table(factor(all_k,levels=1:7))/n,ylab="Sampling Frequency",ylim=c(0,1),col=k.cols)
}
